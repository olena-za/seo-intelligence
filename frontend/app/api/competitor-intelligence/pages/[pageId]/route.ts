...
export async function GET(_request: Request, { params }: { params: Promise<{ pageId: string }> }) {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/competitor-intelligence/pages/${pageId}`, {
      cache: 'no-store',
    });
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: data?.error?.message || data?.message || response.statusText || 'Failed to load ranking-page detail' },
